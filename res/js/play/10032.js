...
_.a0={
  ...
  set:function(data){
    ...
    html.push('<div class="l1 title"><div class="cl1">'+red0.join('')+'</div><div class="cl2">'+blue0.join('')+'</div><div class="code cl3">开奖号码</div></div>');
    h.push('<div class="issu">期号</div>');
    i=D.length-1;
    for(i;i>=0;i--){
      h.push('<div class="issu issu2">'+D[i].issue.substr(4,3)+'</div>');
      ...
      html.push('<div class="cl1">'+red.join('')+'</div><div class="cl5">'+blue.join('')+'</div><div class="code cl3">'+tmpCode+'</div></div>');
      lq.push(t2);
    }
    h.push('<div class="issu mt10 btb">'+this.fetchSize+'期</div><div class="issu">出现</div><div class="issu">遗漏</div><div class="issu">欲出</div>');
    html.push('<div class="splitLine" style="height:'+(27*(D.length+6))+'px;left:824px"></div>');
    //html.push('<canvas id="lines1" width="400" height="'+25*D.length+'"></canvas>');
    html.push('<div class="l1 ls mt10 btb title"><div class="cl1">'+red0.join('')+'</div><div class="cl5">'+blue0.join('')+'</div><div class="code cl3"></div></div>');
    html.push('<div class="l1 ls"><div class="cl1"><em><span>'+data.redappearcount.join('</span></em><em><span>')+'</span></em></div><div class="cl5"><em><span>'+data.blueappearcount.join('</span></em><em><span>')+'</span></em></div><div class="code cl3"></div></div>');
    html.push('<div class="l1 ls"><div class="cl1"><em><span>'+data.redmostomission.join('</span></em><em><span>')+'</span></em></div><div class="cl5"><em><span>'+data.bluemostomission.join('</span></em><em><span>')+'</span></em></div><div class="code cl3"></div></div>');		
    html.push('<div class="l1 ls"><div class="cl1"><em><span>'+data.redappearrate.join('</span></em><em><span>')+'</span></em></div><div class="cl5"><em><span>'+data.blueappearrate.join('</span></em><em><span>')+'</span></em></div><div class="code cl3"></div></div>');
    if(!cp2y.flag){
      var red1=[],blue1=[];
      i=1;
      for(i;i<34;i++){
    ...
    Dom.S.html(html.join('')).css({'width':1405,'height':25*(D.length+1)+150});;Dom.I.html(h.join(''));
    this.setScroll();
    //myScroll2.refresh();
...
_.a1={
  ...
  set:function(data){
    ...
    html.push('<div class="l1 title"><div class="cl1" style="width:'+(w1.length*25)+'px;">第一位</div>');
    html.push('<div class="cl5" style="width:'+(w2.length*25)+'px;">第二位</div>');
    html.push('<div class="cl2" style="width:'+(w3.length*25)+'px;">第三位</div>');
    html.push('<div class="cl3" style="width:'+(w4.length*25)+'px;">第四位</div>');
    html.push('<div class="cl4" style="width:'+(w5.length*25)+'px;">第五位</div>');
    html.push('<div class="cl6" style="width:'+(w6.length*25)+'px;">第六位</div></div>');
    html.push('<div class="l1 title"><div><em>'+w1.join('</em><em>')+'</em></div>');
    ...
    for(i;i<len;i++){
      ...
      h.push('<div class="issu issu2">'+D[len-1-i].issue.substr(4,3)+'</div>');
      t.push('<div class="l1 ls"><div class="cl1">');
      for(j;j<=len2;j++){
        if(j==r1[i]){
...
//    html.push('<canvas id="lineDw3" style="left:'+((w1.length+w2.length)*25)+'px"  width="'+w3.length*25+'" height="'+25*D.length+'"></canvas>');
//    html.push('<canvas id="lineDw4" style="left:'+((w1.length+w2.length+w3.length)*25)+'px"  width="'+w4.length*25+'" height="'+25*D.length+'"></canvas>');
//    html.push('<canvas id="lineDw5" style="left:'+((w1.length+w2.length+w3.length+w4.length)*25)+'px"  width="'+w5.length*25+'" height="'+25*D.length+'"></canvas>');
//    html.push('<canvas id="lineDw6" style="left:'+((w1.length+w2.length+w3.length+w4.length+w5.length)*25)+'px"  width="'+w6.length*25+'" height="'+25*D.length+'"></canvas>');
    html.push('<div class="splitLine" style="height:'+(27*(D.length+6))+'px;left:'+(w1.length)*25+'px"></div>');
    html.push('<div class="splitLine" style="height:'+(27*(D.length+6))+'px;left:'+((w1.length+w2.length)*25-2)+'px"></div>');
    html.push('<div class="splitLine" style="height:'+(27*(D.length+6))+'px;left:'+((w1.length+w2.length+w3.length)*25-3)+'px"></div>');
    html.push('<div class="splitLine" style="height:'+(27*(D.length+6))+'px;left:'+((w1.length+w2.length+w3.length+w4.length)*25-4)+'px"></div>');
    html.push('<div class="splitLine" style="height:'+(27*(D.length+6))+'px;left:'+((w1.length+w2.length+w3.length+w4.length+w5.length)*25-5)+'px"></div>');
    Dom.S.html(html.join('')).css({'width':26+w,'height':25*(D.length+2)});
    Dom.I.html(h.join(''));
    this.setScroll();
...
_.a2={
  ...
  set:function(data){
    ...
    html.push('<div class="l1 title"><div class="hz">和值</div><div class="hz2"></div><div class="code"></div></div>');
    html.push('<div class="l1 title"><div class="hz"><a>21</a><a>10%</a><a>20%</a><a>30%</a><a>40%</a><a>50%</a><a>60%</a><a>70%</a><a>80%</a><a>90%</a><a>183</a></div><div class="hz2">和值</div><div class="code">开奖号码</div></div>');
    h.push('<div class="issu"></div><div class="issu">期号</div>')
    i=D.length-1;
    for(i;i>=0;i--){
      red=D[i].drawNumber.split("#")[0].split(',');
      ...
      sum.push(tSum);
      h.push('<div class="issu issu2">'+D[i].issue.substr(4,3)+'</div>');
      html.push('<div class="l1 ls"><div class="hz cl0"><b style="left:'+((tSum-15)/183)*100+'%">'+tSum+'</b><a></a><a></a><a></a><a></a><a></a><a></a><a></a><a></a><a></a><a></a><a></a></div><div class="hz2 cl5">'+tSum+'</div><div class="code cl6"><a class="red">'+red.join('</a><a class="red">')+'<a class="blue">'+blue+'</a></div></div>');
    }
    html.push('<canvas id="HeZhi" width="440" height="'+25*D.length+'"></canvas>');
    Dom.S.html(html.join('')).css({'width':661,'height':25*(D.length+2)});;Dom.I.html(h.join(''));
    this.setScroll();
